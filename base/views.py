...
class TaskCreateUpdateView(APIView):
    ...
    def put(self, request, pk):
        ...
        if request.user.role == 'User' and task.assigned_to != request.user:
            ...
            return Response({"error": "Managers cannot update completed tasks."}, status=status.HTTP_403_FORBIDDEN)

        serializer = TaskSerializer(task, data=request.data, partial=True, context={'request': request})
        if serializer.is_valid():
            task = serializer.save()
            return Response({"message": "Task updated successfully", "task": TaskSerializer(task).data}, status=status.HTTP_200_OK)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
class TaskDeleteView(APIView):
    ...
    def delete(self, request, pk):
        ...
        if request.user.role == 'User':
            return Response({"error": "Users cannot delete tasks."}, status=status.HTTP_403_FORBIDDEN)
        if request.user.role == 'Manager' and task.status == 'Completed':
            return Response({"error": "Managers cannot delete completed tasks."}, status=status.HTTP_403_FORBIDDEN)
...
class ProjectDeleteView(APIView):
    ...
    def delete(self, request, pk):
        ...
        project.delete()
        return Response({"message": "Project and its tasks deleted successfully."}, status=status.HTTP_204_NO_CONTENT)
...
class ProjectUpdateView(generics.UpdateAPIView):
    ...
    def put(self, request, *args, **kwargs):
        ...
        if request.user.role == 'Manager':
            return Response({"error": "Managers cannot update projects."}, status=status.HTTP_403_FORBIDDEN)
...
class RevokeApprovalView(APIView):
    ...
    def post(self, task_id):
        ...
        return Response({'message': 'Task approval has been revoked and will not be saved to the database.'}, status=200)
