...
class AccessControlMiddleware:
    ...
    def __call__(self, request):
        ...
        public_routes = ['/account/register/', '/admin/', '/account/login/' ]
    ...
    def validate_token(self, token, request):
        ...
        try:
            ...
            if access_start or access_end:
                if access_start and current_time < datetime.datetime.strptime(access_start, '%H:%M').time():
                    raise TokenValidationError("Access not allowed before specified time")
...
class TimeBasedAccessMiddleware:
    ...
    def __call__(self, request):
        ...
        public_routes = ['/account/register/', '/admin/', '/account/login/','/account/logout/','/base/tasks/','/base/tasks/<int:pk>/delete/','/base/projects/<int:pk>/']
        if any(request.path.startswith(route) for route in public_routes):
            return self.get_response(request)
        ...
        if request.body:
            try:
                ...
                username = request_data.get('username')  # Get username from the parsed JSON
        ...
        if not username:
            return JsonResponse({'error': 'Username not provided in the request.'}, status=400)

        try:
            # Retrieve the user by username
            user = User.objects.get(username=username)  # Assuming username is stored in the 'username' field of User model
            user_role = user.role  # Assuming role is stored in the 'role' field of User model
        except User.DoesNotExist:
            return JsonResponse({'error': 'User with this username not found.'}, status=404)
